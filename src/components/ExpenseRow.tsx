...
export default function ExpenseRow({ gasto, onClick, forexRates, dolarBlueRate, privacyMode }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center justify-between gap-3 px-5 py-2.5 text-left active:bg-secondary/35 transition-colors duration-200"
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0 space-y-0.5">
          ...
          <div className="text-[0.91rem] leading-[1.18] font-semibold text-foreground" style={descriptionClampStyle}>{gasto.description}</div>
      ...
      <div className="flex flex-col items-end flex-shrink-0 gap-1">
        <div className={`inline-flex items-center rounded-full px-2.5 py-1.5 text-[0.81rem] font-semibold tabular ${amountPillClass}`}>
